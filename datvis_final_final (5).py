...
# =========================
# KONFIGURASI HALAMAN (WAJIB DI SINI)
# =========================
st.set_page_config(
...
# =========================
VIS_SCALE = 0.5  # 0.5 = setengah ukuran
...
# =========================
    ...
    # Gunakan layout yang lebih kompak dengan kolom yang sama rata
    cols = st.columns(2)
        ...
        with cols[idx % 2]:
            ...
            if len(quartile_texts) > 0:
                ...
                if len(all_text.strip()) > 0:
                    # Buat WordCloud dengan ukuran yang lebih kecil
                    wc = WordCloud(
                        width=300,
                        height=200,
                        background_color='white',
                    ...
                    # Buat figure yang lebih kecil
                    fig, ax = plt.subplots(figsize=(4, 3))
                    ax.imshow(wc, interpolation='bilinear')
                    ax.axis("off")

                    # Gunakan container width untuk kontrol ukuran
                    st.pyplot(fig, use_container_width=True)

                    # Info statistik di bawah gambar
                    mean_protein = df_preprocessed_numeric.loc[idx_quartile, 'Protein_g'].mean()
                    st.caption(f"Rata-rata protein: {mean_protein:.1f}g | {len(quartile_texts)} makanan")
    ...
    # Statistik ringkas dengan container yang lebih kecil
    st.divider()
    st.subheader("📊 Statistik Ringkas")

    # Tampilkan statistik dalam dataframe yang lebih kecil
    quartile_stats = []
...
# =========================
    ...
    # Buat container untuk visualisasi yang lebih kecil
    with st.container():
        ...
        ax.set_title(f'Hubungan {x_axis} dan {y_axis}', fontsize=10, fontweight='bold', pad=5)
        ax.set_xlabel(x_axis.replace('_', ' '), fontsize=9, labelpad=2)
        ax.set_ylabel(y_axis.replace('_', ' '), fontsize=9, labelpad=2)
        ax.tick_params(labelsize=8)
        ax.grid(True, alpha=0.2, linewidth=0.5)

        # Legend yang lebih kecil dan rapi
        ...
        # Gunakan container width untuk membatasi ukuran
        st.pyplot(fig, use_container_width=True)
    ...
    with st.expander("📝 Interpretasi", expanded=False):
        ...
        **Nilai:** {correlation:.3f}

        **Hubungan:** {direction} ({strength.lower()})

        **Implikasi:** {f"Jika {x_axis.replace('_', ' ')} meningkat, {y_axis.replace('_', ' ')} cenderung " + ("meningkat" if correlation > 0 else "menurun")}
        """)

    # Data point outlier dalam expander
    with st.expander("📊 Data Nilai Ekstrem", expanded=False):
        col1, col2 = st.columns(2)

        with col1:
            st.markdown("**5 Data Tertinggi Sumbu X:**")
            top_x = df_preprocessed_numeric.nlargest(5, x_axis)[['Food_Name', 'Label_Kesehatan', x_axis, y_axis]]
            top_x.columns = ['Nama Makanan', 'Label', x_axis, y_axis]
            st.dataframe(top_x, use_container_width=True, height=150)

        with col2:
            st.markdown("**5 Data Tertinggi Sumbu Y:**")
            top_y = df_preprocessed_numeric.nlargest(5, y_axis)[['Food_Name', 'Label_Kesehatan', x_axis, y_axis]]
            top_y.columns = ['Nama Makanan', 'Label', x_axis, y_axis]
            st.dataframe(top_y, use_container_width=True, height=150)

...
# 5. Halaman Analisis Multi-Dimensi
    ...
    with tab3:
        ...
        for idx, category in enumerate(categories):
            with st.expander(f"🔍 **{category.upper()}**", expanded=(idx==0)):
                ...
                for nutrient, name, unit in zip(nutrients, nutrient_names, units):
                    ...
                    avg_values.append({
                        'Nutrisi': name,
                        'Rata-rata': f"{avg_val:.1f} {unit}",
                        'Nilai': avg_val
                    })

                # Sort berdasarkan nilai
                avg_df = pd.DataFrame(avg_values)
                avg_df = avg_df.sort_values('Nilai', ascending=False)
                ...
                for i, row in enumerate(avg_df.itertuples()):
                    with cols[i]:
                        st.metric(
                            label=row.Nutrisi,
                            value=row.Rata_rata.split()[0],
                            delta=row.Rata_rata.split()[1],
                            delta_color="off"
                        )
        ...
        # Plot perbandingan
        fig, ax = plt.subplots(figsize=(8, 5))
        colors = [colors[cat] for cat in comparison_df['Kategori']]
        bars = ax.bar(comparison_df['Kategori'], comparison_df['Rata-rata'], color=colors)
        ...
        sorted_df = comparison_df.sort_values('Rata-rata', ascending=False)
        ...
        **{selected_nutrient} tertinggi** ditemukan pada kategori **{highest['Kategori']}**
        dengan rata-rata **{highest['Rata-rata']:.1f} {unit}**

        **{selected_nutrient} terendah** ditemukan pada kategori **{lowest['Kategori']}**
        dengan rata-rata **{lowest['Rata-rata']:.1f} {unit}**

        **Perbedaan:** {abs(highest['Rata-rata'] - lowest['Rata-rata']):.1f} {unit}
        """)

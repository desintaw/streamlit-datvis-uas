...
# 7. DISTRIBUSI ENERGI
# =========================
elif page == "Distribusi Energi":
    st.header("Distribusi Energi (Kalori) Makanan")

    st.subheader("Data Setelah Preprocessing")

    # Visualisasi distribusi energi setelah preprocessing
    ...
    ax.axvline(mean_val, color='red', linestyle='--', label=f'Mean: {mean_val:.1f}', linewidth=2)
    ax.axvline(median_val, color='green', linestyle='--', label=f'Median: {median_val:.1f}', linewidth=2)
    ax.legend(fontsize=9)

    st.pyplot(fig, use_container_width=False)

    # Statistik ringkas
    st.subheader("Statistik Distribusi Energi")

    energy_stats = df_numeric_capped["Energy_kcal"].describe()
    stats_df = pd.DataFrame({
        'Statistik': ['Mean', 'Std Dev', 'Minimum', '25%', '50% (Median)', '75%', 'Maksimum'],
        'Nilai (kcal)': [
            f"{energy_stats['mean']:.1f}",
            f"{energy_stats['std']:.1f}",
            f"{energy_stats['min']:.1f}",
            f"{energy_stats['25%']:.1f}",
            f"{energy_stats['50%']:.1f}",
            f"{energy_stats['75%']:.1f}",
            f"{energy_stats['max']:.1f}"
        ]
    })

    st.dataframe(stats_df, use_container_width=True, height=250, hide_index=True)

    # Informasi tentang preprocessing
    st.subheader("Informasi Preprocessing")
    st.info("""
    **Preprocessing yang dilakukan pada data energi:**
    1. **Penanganan Missing Values:** Mengisi nilai yang hilang dengan median
    2. **Penanganan Outlier:** Menggunakan metode IQR (Interquartile Range) untuk mengidentifikasi dan meng-clip outlier
    3. **Scaling:** Data distandarisasi menggunakan StandardScaler (untuk analisis lain)

    **Parameter clipping berdasarkan IQR:**
    - Q1 (25th percentile): {:.1f} kcal
    - Q3 (75th percentile): {:.1f} kcal
    - IQR: {:.1f} kcal
    - Batas bawah: {:.1f} kcal
    - Batas atas: {:.1f} kcal
    """.format(
        df_numeric_capped["Energy_kcal"].quantile(0.25),
        df_numeric_capped["Energy_kcal"].quantile(0.75),
        df_numeric_capped["Energy_kcal"].quantile(0.75) - df_numeric_capped["Energy_kcal"].quantile(0.25),
        df_numeric_capped["Energy_kcal"].quantile(0.25) - 1.5 * (df_numeric_capped["Energy_kcal"].quantile(0.75) - df_numeric_capped["Energy_kcal"].quantile(0.25)),
        df_numeric_capped["Energy_kcal"].quantile(0.75) + 1.5 * (df_numeric_capped["Energy_kcal"].quantile(0.75) - df_numeric_capped["Energy_kcal"].quantile(0.25))
    ))

# =========================
# 8. NILAI MAKSIMUM
# =========================
elif page == "Nilai Maksimum":
    st.header("Nilai Maksimum Kandungan Nutrisi")
    ...
    st.subheader("Data Nilai Maksimum")

    # Buat DataFrame dari max_values
    max_df = pd.DataFrame({
    ...
    st.dataframe(max_df, use_container_width=True, height=250, hide_index=True)

    # Tampilkan makanan dengan nilai maksimum
    st.subheader("Makanan dengan Kandungan Tertinggi")

    col1, col2 = st.columns(2)

    with col1:
        # Makanan dengan lemak tertinggi
        max_fat = df.loc[df['Fat_g'].idxmax()]
        st.metric(
            "Lemak Tertinggi",
            f"{max_fat['Fat_g']:.1f} g",
            max_fat['Food_Name'][:30] + ("..." if len(max_fat['Food_Name']) > 30 else "")
        )

    with col2:
        # Makanan dengan gula tertinggi
        max_sugar = df.loc[df['Sugar_g'].idxmax()]
        st.metric(
            "Gula Tertinggi",
            f"{max_sugar['Sugar_g']:.1f} g",
            max_sugar['Food_Name'][:30] + ("..." if len(max_sugar['Food_Name']) > 30 else "")
        )
    ...
    with col3:
        ...
        max_protein = df.loc[df['Protein_g'].idxmax()]
        st.metric(
            "Protein Tertinggi",
            f"{max_protein['Protein_g']:.1f} g",
            max_protein['Food_Name'][:30] + ("..." if len(max_protein['Food_Name']) > 30 else "")
        )

    with col4:
        # Makanan dengan natrium tertinggi
        max_sodium = df.loc[df['Sodium_mg'].idxmax()]
        st.metric(
            "Natrium Tertinggi",
            f"{max_sodium['Sodium_mg']:.1f} mg",
            max_sodium['Food_Name'][:30] + ("..." if len(max_sodium['Food_Name']) > 30 else "")
        )

# =========================
# 9. TOP 10 PER KATEGORI
# =========================
elif page == "Top 10 per Kategori":
    st.header("Top 10 Makanan Berdasarkan Data yang Sudah Dipreprocessing")
    ...
    st.subheader("Data Detail Top 10 per Kategori")

    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
        "Protein Sehat", "Gula Kurang Sehat", "Lemak Cukup Sehat",
        "Karbohidrat Sehat", "Energi Kurang Sehat", "Natrium Kurang Sehat"
    ])

    with tab1:
        st.dataframe(sehat_high_protein[['Food_Name', 'Protein_g', 'Label_Kesehatan']],
                    use_container_width=True, height=300)

    with tab2:
        st.dataframe(kurang_sehat_high_sugar[['Food_Name', 'Sugar_g', 'Label_Kesehatan']],
                    use_container_width=True, height=300)

    with tab3:
        st.dataframe(cukup_sehat_high_fat[['Food_Name', 'Fat_g', 'Label_Kesehatan']],
                    use_container_width=True, height=300)

    with tab4:
        st.dataframe(sehat_high_carbo[['Food_Name', 'Carbohydrate_g', 'Label_Kesehatan']],
                    use_container_width=True, height=300)

    with tab5:
        st.dataframe(kurang_sehat_high_energy[['Food_Name', 'Energy_kcal', 'Label_Kesehatan']],
                    use_container_width=True, height=300)

    with tab6:
        st.dataframe(kurang_sehat_high_sodium[['Food_Name', 'Sodium_mg', 'Label_Kesehatan']],
                    use_container_width=True, height=300)
